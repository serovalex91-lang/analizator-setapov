...
async def _process_event(event):
    await asyncio.sleep(3)  # задержка 3 секунды перед обработкой любого сообщения
    try:
        ...
        if src_ok and message:
            ...
            for line in message.splitlines():
                ...
                if _is_correction_combo(squares):
                    ...
                    # Отправка графика
                    await asyncio.sleep(10) # задержка перед отправкой графика
                    try:
                        chart_req = {
                            "symbol": symbol_usdt,
                            "origin_tf": origin_tf,
                ...
                if _is_resistance_combo(squares):
                    ...
                    await asyncio.sleep(10)  # задержка перед отправкой вебхука
                    try:
                        await _send_webhook_from_level(
                            symbol_usdt, "sell",
                            last_price, sl_adjusted, tp_target, (rng_low, rng_high)
